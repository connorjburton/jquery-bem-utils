...
/***/ function(module, exports, __webpack_require__) {
	...
	(function (root, factory) {
		...
		var BEM = function () {
				...
				this.regex = {
					block: function block(name) {
						if (!name.length) {
							...
							return name + '(?!' + _this.options.prefixes.element + ')';
						}
					},
					element: function element(name) {
						if (!name.length) {
							...
							return _this.options.prefixes.element + name + '(?!' + _this.options.prefixes.element + ')';
						}
					},
					modifiers: function modifiers() {
						return _this.options.prefixes.modifier + '[a-z0-9]*[-_]?[a-z0-9]*';
					},
					modifier: function modifier(mod) {
						if (!mod.length) {
							console.error('jQuery BEM Utils: No modifier supplied in BEM.regex.modifier');
						} else {
							return _this.options.prefixes.modifier + mod + '$';
						}
					}
				};
			}
			...
			_createClass(BEM, [{
				key: 'findBlock',
				value: function findBlock(name) {
					var _this2 = this;

					return $('[class^=' + name + '"').filter(function () {
						return _this2.className.match(RegExp(_this2.regex.block(name), 'g'));
					});
				}
			}, {
				key: 'findElement',
				value: function findElement(name) {
					...
					return $(this).find('[class*="' + name + '"]').filter(function () {
						return _this3.className.match(RegExp(_this3.regex.element(name), 'g'));
					});
				}
			}, {
				key: 'addModifier',
				value: function addModifier(mod, unique) {
					var $this = $(this);
					if (typeof mod === 'undefined' || !$this.length) return;

					if (!$this.hasModifier(mod)) {
						var newClass = $this.attr('class') + this.options.prefixes.modifier + mod;
				...
				key: 'removeModifier',
				value: function removeModifier(mod) {
					var $this = $(this);
				...
				value: function getModifiers() {
					...
					var matches = $this.attr('class').match(RegExp(this.regex.modifiers(), 'gi'));

					return matches.map(function (str) {
						return str.split('').splice(_this4.options.prefixes.modifier.length, str.length).join('');
					});
				}
			}, {
				key: 'hasModifier',
				value: function hasModifier(mod) {
					var $this = $(this);
